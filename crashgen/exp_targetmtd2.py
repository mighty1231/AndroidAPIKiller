...
def ape_task(avd_name, serial, package_name, output_dir, running_minutes, mt_is_running, mtdtarget, no_guide):
    ...
    def ape_stdout_callback(line):
        ...
        if line.startswith("[APE_MT] mt data "):
            prefix = line[len("[APE_MT] mt data "):]
            os.path.split(prefix)
            out = run_adb_cmd("shell ls {}*".format(prefix), serial=serial)
            if "No such file or directory" not in out:
                ...
                for fname in fnames:
                    run_adb_cmd("pull {} {}".format(fname, mt_data_dir), serial=serial)
                    print("callback: {} pulled into {}".format(fname, mt_data_dir))
                    run_adb_cmd("shell rm {}".format(fname), serial=serial)
            else:
                print("Warning: No such file or directory - {}*".format(prefix))
